...
#define modbus_rtu_write           modbus_write
#define modbus_rtu_read            modbus_read
#define modbus_rtu_init            modbus_init
#define modbus_rtu_done            modbus_done
#define modbus_rtu_connect         modbus_connect
#define modbus_rtu_listen          modbus_listen
#define modbus_rtu_close           modbus_close
#define modbus_rtu_silence_init    modbus_silence_init
#define modbus_rtu_get_min_timeout modbus_get_min_timeout
...
static int nd_entry_connect(nd_entry_t *nde,
  ...
  */
  ...
  if (termios_init(&settings,
      ...
      < 0) {
    plc_log_errmsg(1, "Invalid serial line settings"
  ...
  if((nde->fd = open(node_addr->addr.rtu.device, O_RDWR | O_NOCTTY | O_NDELAY))
     < 0) {
    plc_log_errmsg(1, "Error opening device %s (errno=%d)",
                   node_addr->addr.rtu.device, errno);
    goto error_exit_3;
  }

  if(tcgetattr(nde->fd, &nde->old_tty_settings_) < 0) {
    plc_log_errmsg(1, "Error reading device's %s original settings.",
                      node_addr->addr.rtu.device);
    goto error_exit_4;
  }

  if(tcsetattr(nde->fd, TCSANOW, &settings) < 0) {
    plc_log_errmsg(1, "Error configuring device %s"
                      "(baud=%d, parity=%d, data_bits=%d, stop_bits=%d)",
                      node_addr->addr.rtu.baud,
                      node_addr->addr.rtu.parity,
                      node_addr->addr.rtu.data_bits,
  ...
  nde->time_15_char_ = d_to_timeval(1.5*char_bits/node_addr->addr.rtu.baud);
  nde->time_35_char_ = d_to_timeval(3.5*char_bits/node_addr->addr.rtu.baud);
...
static int nd_entry_free(nd_entry_t *nde) {
  ...
  if(tcsetattr(nde->fd, TCSANOW, &nde->old_tty_settings_) < 0)
    plc_log_wrnmsg(1, "Error reconfiguring serial port to it's original settings.");
...
#ifdef DEBUG
  printf("\n" );
  printf("returning valid frame of %d bytes.\n", frame_length);
#endif
                             ...
                             u8 *slave_id)
{
  ...
  *recv_data_ptr = NULL;
   ...
   */
    FD_ZERO(&rfds);
    FD_SET(nd_entry->fd, &rfds);
    {int sel_res = my_select(nd_entry->fd + 1, &rfds, end_time);
      if (sel_res < 0)
        return -1;
      if (sel_res == 0)
        return -2;
    ...
    if (recv_buf->found_frame_boundary == 0) {
      ...
      {/* display the hex code of each character received */
        ...
        for (i=0; i < read_stat; i++)
          printf("<0x%2X>", *(lb_free(&recv_buf->data_buf) + i));
      }
#endif
                    ...
                    int *extra_bytes)
{
  ...
  if (crc_init(opt) < 0) {
    plc_log_errmsg(1, "Out of memory: error initializing crc buffers");
    goto error_exit_0;
  }
  ...
  *extra_bytes = RTU_FRAME_CRC_LENGTH;
    ...
    /* allocate nd_table_ */
  if ((nd_table_ = malloc(sizeof(nd_table_t))) == NULL) {
...
#ifdef DEBUG
  printf("modbus_rtu_init(): returning succesfuly...\n");
#endif
  return 0;
...
#ifdef DEBUG
  printf("modbus_rtu_connect(): called...\n");
  printf("opening %s\n", node_addr.addr.rtu.device);
  printf("baud_rate = %d\n", node_addr.addr.rtu.baud);
  printf("parity = %d\n", node_addr.addr.rtu.parity);
  printf("data_bits = %d\n", node_addr.addr.rtu.data_bits);
...
#ifdef DEBUG
  printf("modbus_rtu_connect(): %s open\n", node_addr.addr.rtu.device);
  printf("modbus_rtu_connect(): returning nd=%d\n", node_descriptor);
#endif
  return node_descriptor;

  error_exit_0:
#ifdef DEBUG
  printf("modbus_rtu_connect(): error!\n");
#endif
    return -1;
}
