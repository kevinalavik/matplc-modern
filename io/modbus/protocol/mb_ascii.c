...
#define modbus_ascii_write           modbus_write
#define modbus_ascii_read            modbus_read
#define modbus_ascii_init            modbus_init
#define modbus_ascii_done            modbus_done
#define modbus_ascii_connect         modbus_connect
#define modbus_ascii_listen          modbus_listen
#define modbus_ascii_close           modbus_close
#define modbus_ascii_silence_init    modbus_silence_init
#define modbus_ascii_get_min_timeout modbus_get_min_timeout
...
static int nd_entry_connect(nd_entry_t *nde,
  ...
  */
  ...
  if (send_buf_init(&nde->send_buf_, buf_size, buf_size - SEND_BUF_MIN_LENGTH)
      == NULL) {
    plc_log_errmsg(1, "Out of memory: error initializing send buffer");
    goto error_exit_1;
  }
                      ...
                      int *extra_bytes)
{
  ...
  if (nd_count <= 0)
     ...
     * before calling modbus_ascii_write()
     */
  if (extra_bytes != NULL)
    *extra_bytes = 0;
