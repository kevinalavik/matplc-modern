...
int	parser::parse(char *string, char delimeter)
{
	...
	if(tokens_string == NULL){
		error_log("ERROR PAR1: Out of Memory");
		error = ERROR;
		//exit(1);
	}
	error = NO_ERROR;
		...
		if(((string[i] != delimeter)
			...
			if(flag1 == 0){
				...
				if(count >= pnt_end_list){
					error_log("ERROR PAR2: Out of Memory");
					error = ERROR;
					//exit(1);
				}
				tokens[count] = new char[strlen(tokens_string) + 1];
				if(tokens[count] == NULL){
					error_log("ERROR PAR3: Out of Memory");
					error = ERROR;
					//exit(1);
				}
	...
	if((count >= 0) && (strlen(tokens_string) < 1)){
		...
		tokens_string[pos] = 0;
		if(count >= pnt_end_list){
			error_log("ERROR PAR4: Out of Memory ");
			exit(1);
		}
		tokens[count] = new char[strlen(tokens_string) + 1];
		if(tokens[count] == NULL){
			error_log("ERROR PAR5: Out of Memory");
			error = ERROR;
			//exit(1);
		}
		strcpy(tokens[count], tokens_string);
...
char* parser::token(int number)
{
	if((number >= 0) && (number <= end_of_list)){
		...
		error_log("ERROR PAR6: Token index outside list");
		return NULL;
	}
}
