...
typedef struct {
  ...
  const char *on, *off; /* markers for on and off when data type is bool */
...
typedef struct {
  char *name;
  u32 max_x, min_x; /* position on screen for horizontal axis */
  u32 max_y, min_y; /* position on screen for vertical   axis */
...
/* Function to pad out a string with spaces to the given length. The new
 * string is malloc(3)ed. Don't forget to free(3) the old string if you
 * need to.
 */
char *space_pad(const char *s, int len)
{
  char *res; int slen;
  slen = strlen(s);
  if (slen>len) {
...
void get_config(void)
{
  ...
  for (i = 0; i < disp_value_items; i++) {
    ...
    if (s) {
      ...
      disp_value[i].on = strdup("1");
    s = conffile_get_table("show",i,next_table_pos);
    if (s) {
      disp_value[i].off = s;
    } else
      disp_value[i].off = strdup("0");

    /* make sure the strings are the same length... */
    if (strlen(disp_value[i].on) < strlen(disp_value[i].off)) {
      s = (char*)disp_value[i].on;
      disp_value[i].on = space_pad(s, strlen(disp_value[i].off));
      free(s);
    }
    else if (strlen(disp_value[i].on) > strlen(disp_value[i].off)) {
      s = (char*)disp_value[i].off;
      disp_value[i].off = space_pad(s, strlen(disp_value[i].on));
      free(s);
    }
  }
...
f32 pt_to_f32(plc_pt_t pt, data_type_t dt)
{
  ...
  switch (dt) {
    ...
    case i32_dt:
      return tmp.i;
    case  u8_dt: case u16_dt: case u32_dt:
      return tmp.u;
    default : break;
  }; /* switch() */
...
void show(void)
{
  ...
  union {f32 f; u32 u; i32 i;} tmp;

  for (i = 0; i < disp_value_items; i++) {
    if (disp_value[i].data.data_type == bool_dt)
      if (plc_get(disp_value[i].data.pt))
	mvaddstr(disp_value[i].y, disp_value[i].x, disp_value[i].on);
      else
	mvaddstr(disp_value[i].y, disp_value[i].x, disp_value[i].off);
    else {
      tmp.u = plc_get(disp_value[i].data.pt);
      switch (disp_value[i].data.data_type) {
        case f32_dt:
	  snprintf(str_buff, BUFF_SIZE, "%9f", tmp.f); break;
        case  i8_dt:
	  if (tmp.i>127)
	    snprintf(str_buff, BUFF_SIZE, "%d", 256-tmp.i);
	  else
	    snprintf(str_buff, BUFF_SIZE, "%d", tmp.i);
	  break;
        case i16_dt:
	  if (tmp.i>32767)
	    snprintf(str_buff, BUFF_SIZE, "%d", 32768-tmp.i);
	  else
	    snprintf(str_buff, BUFF_SIZE, "%d", tmp.i);
	  break;
        case  u8_dt: case u16_dt: case u32_dt:
          snprintf(str_buff, BUFF_SIZE, "%d", tmp.u); break;
        case i32_dt:
	  snprintf(str_buff, BUFF_SIZE, "%d", tmp.i); break;
	case bool_dt:
	  strcpy(str_buff, tmp.u ? "1" : "0"); break;
      }; /* switch() */
...
{
  if (plc_init("vitrine", argc, argv) < 0) {
    printf("Error connecting to PLC.\n");
    exit(1);
  }

  initscr(); /* required so that the LINES and COLS variables of ncurses are initialized */
  get_config();
  dump_config();
  initialize_screen();
  while (1) {
    ...
    show();
      ...
      /* this would only be required if this module changed any plc points...*/
    /* plc_update(); */
    plc_scan_end();
  }
}
