...
/* DEBUGING */
#define DEBUG_gnome_pixmap_widget
#undef DEBUG_gnome_pixmap_widget

#define DEBUG_hmi_GnomePixmap_read_matplc_conf
#undef DEBUG_hmi_GnomePixmap_read_matplc_conf
...
/* includes for GtkSocket */
#ifdef GTKSOCKET
#include <gtk-1.2/gdk/gdkx.h>
#endif


/* includes for GtkPlotCanvas */
#ifdef GTKEXTRA
	#include <gtkplot.h>
	...
	#define DEFAULT_YMAX 20
	#define DEFAULT_SAMPLETIME  0.5
	#define DEFAULT_WIDGET_HEIGHT 600
	#define DEFAULT_WIDGET_WIDTH  600
	#define DEFAULT_SYMBOLSIZE	2
	#define DEFAULT_LINEWIDTH	1
	#define AXIS_TICKS_MAJOR	0.1
	#define AXIS_TICKS_MINOR	1
#endif
...
/* Variables for GtKPlotCanvas */
	typedef enum {false=0,true=1} t_boolean ;
#ifdef GTKEXTRA
	typedef enum {timermode=0,plotxymode=1} mode ;
	typedef enum {notconfigured=0,configured=1} configuration ;
	...
	typedef struct{
		...
		GtkPlotSymbolType symboltype;
		GtkPlotSymbolType symbolstyle;
		unsigned int	  symbolsize;
		GtkPlotSymbolType linestyle;
		unsigned int	  linewidth;
		GtkPlotSymbolType pointconnect;
	} channel_t;
	...
	typedef struct{
		...
		configuration conf_widget;
		configuration conf_axis;
		configuration conf_color;
		configuration conf_timer;
		configuration conf_plotctrl;
		configuration conf_plotx;
		configuration conf_channel;
		configuration conf_channelcolor;
		configuration conf_mode;
		configuration conf_symboltype;
		configuration conf_symbolstyle;
		configuration conf_symbolsize;
		configuration conf_linestyle;
		configuration conf_linewidth;
		configuration conf_pointconnect;
	} plot_t;
#endif

/* data type to store GnomePixmap config data */
typedef struct{
	int	nbPixmap;
	char *widgetpattern;
	char **Pixmap;
} hmi_GnomePixmap_tab_t;

typedef struct{
	int	nbPixmapTab;
	hmi_GnomePixmap_tab_t *PixmapTab;
} hmi_GnomePixmap_t;
...
/* Global Config Data for GnomePixmap */
static hmi_GnomePixmap_t hmi_GnomePixmap;
...
/* Just a spaceholder kind of a handler */
void
do_nothing(GtkWidget *w)
{
...
/* Load a GnomePixmap widget image and resize it */
void
load_gnome_pixmap(GtkWidget *widget, char *name){
  gint width, height;
  GtkArg *gtk_arg;

  gtk_arg = malloc(sizeof(GtkArg));
  gtk_arg->name = g_strdup("width");
  gtk_widget_get(widget, gtk_arg);
  width = GTK_VALUE_INT(*gtk_arg);
  g_free(gtk_arg->name);
  gtk_arg->name = g_strdup("height");
  gtk_widget_get(widget, gtk_arg);
  height = GTK_VALUE_INT(*gtk_arg);
  if((width > 0) && (height > 0)){
    gnome_pixmap_load_file_at_size(GNOME_PIXMAP(widget), 
				   name, width, height);
  }
  else {
    gnome_pixmap_load_file(GNOME_PIXMAP(widget), name);
  }
  g_free(gtk_arg->name);
  free(gtk_arg);
}
...
void
gnome_pixmap_widget(connection_t *connection){
  char *image_name;
  char *image;
  int  iPtab=0;
  union {
...
#ifdef DEBUG_gnome_pixmap_widget
...
#ifdef DEBUG_gnome_pixmap_widget
  printf("gnome_pixmap_widget: iPtab=%d\n",iPtab);	  	
#endif
  
  image=NULL;
  if (connection->PixmapTab != NULL) {
	if ((connection->PixmapTab->nbPixmap)>0) {
  		iPtab=iPtab % connection->PixmapTab->nbPixmap;
		image=connection->PixmapTab->Pixmap[iPtab];
	}
  } 
  else {
...
#ifdef DEBUG_gnome_pixmap_widget
	  printf("gnome_pixmap_widget: image_name=%s\n",image_name);
#endif
	  load_gnome_pixmap(connection->widget, image_name);
	  g_free(image_name);
  }
#undef u32_tmp
}
...
/*
	hmi_GnomePixmap_read_matplc_conf()
	Read configuration from matplc.conf
	
	[hmi_gtk]
	GnomePixmap widgetname pixmap1 ... pixmapn
	
*/
void hmi_GnomePixmap_read_matplc_conf(void)
{
	int nbofrows=0;
	int nbofcols=0;
	int iCol,iRow,iWtab,iPtab,found;
	char tablename[]="GnomePixmap";
	char *widgetpattern;
	char *sPixmap=NULL;
	char *image_name;
	FILE *fPixmap;
	
#ifdef DEBUG_hmi_GnomePixmap_read_matplc_conf
	printf("hmi_GnomePixmap_read_matplc_conf()\n");
#endif

	hmi_GnomePixmap.nbPixmapTab=0;
	hmi_GnomePixmap.PixmapTab=NULL;

	nbofrows=conffile_get_table_rows(tablename);
	//printf("GnomePixmap:: nbofrows %d\n",nbofrows);
	...
	for(iRow=0;iRow<nbofrows;iRow++){
		...
		//printf("GnomePixmap:: Widgetname=%s nbofcols=%d\n",
		//	widgetpattern,nbofcols-1);

		/* pruefen ob der Eintrag schon vorhanden ist */
		found=false;
		for (iWtab=0;iWtab<hmi_GnomePixmap.nbPixmapTab;iWtab++) {
			//printf("pruefen iWtab=%d ... ",iWtab);
			if (strcmp(hmi_GnomePixmap.PixmapTab[iWtab].widgetpattern,widgetpattern)==0) {
				found=true;
				free(widgetpattern);
				//printf("found\n");
				break;
		...
		if (!found) {
			hmi_GnomePixmap.nbPixmapTab++;
			hmi_GnomePixmap.PixmapTab=(hmi_GnomePixmap_tab_t*)g_realloc(hmi_GnomePixmap.PixmapTab,
				(hmi_GnomePixmap.nbPixmapTab*sizeof(hmi_GnomePixmap_tab_t)));
				
			//printf("g_realloc: nbPixmapTab=%d iWtab=%d\n",
			//	hmi_GnomePixmap.nbPixmapTab,iWtab);
			
			hmi_GnomePixmap.PixmapTab[iWtab].nbPixmap=0;
			hmi_GnomePixmap.PixmapTab[iWtab].widgetpattern=widgetpattern;
			hmi_GnomePixmap.PixmapTab[iWtab].Pixmap=NULL;
		}
		
		//printf("nbPixmap=%d\n",
		//	hmi_GnomePixmap.PixmapTab[iWtab].nbPixmap
		//	);

		iPtab=hmi_GnomePixmap.PixmapTab[iWtab].nbPixmap;
		hmi_GnomePixmap.PixmapTab[iWtab].nbPixmap+=(nbofcols-1);
		hmi_GnomePixmap.PixmapTab[iWtab].Pixmap=
			(char**)g_realloc( hmi_GnomePixmap.PixmapTab[iWtab].Pixmap,
			hmi_GnomePixmap.PixmapTab[iWtab].nbPixmap*sizeof(char*) );

		//printf("iPtab=%d nbPixmap=%d\n",
		//	iPtab,
		//	hmi_GnomePixmap.PixmapTab[iWtab].nbPixmap
		//	);
		
		for (iCol=1;iCol<nbofcols;iCol++) {
			hmi_GnomePixmap.PixmapTab[iWtab].Pixmap[iPtab]=NULL;
			sPixmap=conffile_get_table(tablename,iRow,iCol);
			image_name = g_strdup_printf("%s.xpm",sPixmap);
			if ((fPixmap=fopen(image_name,"r"))!=NULL) {
				fclose(fPixmap);
				//printf("iCol=%d Pixmap=%s\n",iCol,sPixmap);
				hmi_GnomePixmap.PixmapTab[iWtab].Pixmap[iPtab]=sPixmap;
				iPtab++;
			} else {
				printf("ERROR: Pixmap %s not available!!\n",image_name);
				hmi_GnomePixmap.PixmapTab[iWtab].nbPixmap--;
			}
			g_free(image_name);
		}		
...
#ifdef DEBUG_hmi_GnomePixmap_read_matplc_conf
	for (iWtab=0;iWtab<hmi_GnomePixmap.nbPixmapTab;iWtab++) {
		printf("%d: nbPixmap=%d wpat=%s Pixmap=",
			iWtab,
			hmi_GnomePixmap.PixmapTab[iWtab].nbPixmap,
			hmi_GnomePixmap.PixmapTab[iWtab].widgetpattern);
		
		for (iPtab=0; iPtab<hmi_GnomePixmap.PixmapTab[iWtab].nbPixmap;iPtab++) {
			printf(" %s",hmi_GnomePixmap.PixmapTab[iWtab].Pixmap[iPtab]);
		}
		printf("\n");	
	}
#endif		
...
option_menu_widget_init(connection_t *connection){
	...
	tablename=gtk_widget_get_name(connection->widget);
...
parse_connection(gpointer a_connection, gpointer data){
   ...
   */
  type = GTK_OBJECT(connection->widget)->klass->type;
...
#undef DEBUG_INIT_WIDGET_DATA
	...
	if(widget){
		...
		/* Interim code to automaticaly adjust GnomePixmap to the widget size  
		 * Glade and libglade are currently not handling GnomePixmap image scaling 
		 */
		type = GTK_OBJECT(widget)->klass->type;
		if(type == gtk_type_from_name("GnomePixmap")){
			if(widget_name[0] == '.'){
				split_str = g_strsplit(widget_name+1, "_", 2);
				if(split_str[0]){
					load_gnome_pixmap(GTK_WIDGET(widget), split_str[0]);
				}
				g_free(split_str);
			}
			/* Load a transparent pixmap to avoid painting a large 
			* non scaled pixmap when using Pixmap as display.
			*/
			else if(widget_name[0] == '_'){
				PixmapName = g_strdup("Transparent.xpm");
				load_gnome_pixmap(GTK_WIDGET(widget), PixmapName);
				g_free(PixmapName);
			}
		}
	...
	/* for GnomePixmap find the corresponding PixmapTab Entry */
    if(type == gtk_type_from_name("GnomePixmap")){
		for (i=0;i<hmi_GnomePixmap.nbPixmapTab;i++) {
			if (fnmatch(hmi_GnomePixmap.PixmapTab[i].widgetpattern,split_str[0],0)==0){

#ifdef DEBUG_INIT_WIDGET_DATA
				printf("init_widget_data(): PixmapTab[%d]=%s\n",
					i,hmi_GnomePixmap.PixmapTab[i].widgetpattern);
#endif
				connection->PixmapTab=&hmi_GnomePixmap.PixmapTab[i];
			}
		}
	}
 ...
 */
void
exit_window(GtkWidget *w, gpointer data1, gpointer data2)
{
  if(w){
    // gtk_grab_remove(w);
    gtk_widget_hide(w);
...
#undef DEBUG_RUN_WINDOW
  ...
  if(found_it){
    ...
    xml = glade_xml_new("hmi_gtk.glade", name);
...
main(int argc, char *argv[])
{
  GladeXML *xml;
  GtkWidget *window;
  const char *module_name = "hmi_gtk";
  char *name;
...
#undef DEBUG_MAIN
  ...
  /* initialize gnome */
  gnome_init("hmi_gtk", VERSION, argc, argv);
  ...
  gtk_signal_connect_full(GTK_OBJECT(window), "destroy",
			GTK_SIGNAL_FUNC(exit_window), NULL,
			xml, (GtkDestroyNotify)gtk_object_unref,
			FALSE, FALSE);
