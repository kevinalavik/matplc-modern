...
void plc_timer_clear_fn(plc_timer_t * tp)
{
...
void plc_timer_start_fn(plc_timer_t * tp)
{
  if (sizeof(plc_timer_t) != sizeof(plc_timer_private_t)) {
    printf("Error in timer library - type size mismatch!!!\n");
    exit(1);
  }
  gettimeofday(&(ttt.tv), 0);
  ttte = 1;
}
...
void plc_timer_enable_fn(plc_timer_t * tp)
{
  struct timeval now;
...
void plc_timer_disable_fn(plc_timer_t * tp)
{
  struct timeval now;
 ...
 */
void plc_timer_add_fn(plc_timer_t * tp, double incr)
{
  int s;
  if (ttte)
    incr = -incr;
 ...
 * remain unchanged.
 */
void plc_timer_set_fn(plc_timer_t * tp, double val)
{
  if (ttte) {
    int s;
    gettimeofday(&(ttt.tv), 0);
...
inline double plc_timer_get(plc_timer_t t)
{
  if (tte) {
    struct timeval now;
    gettimeofday(&now, 0);
...
{
    return plc_timer_get(timer) >= preset;
}
