...
int	step_run(){
	...
	if(count == 5){
		sprintf(temp, "MESSAGE clientA MESSAGE THIS a message from pid=%d ", getpid());
		error = send_message(temp);
	}
	if(count == 10){
		sprintf(temp, "MESSAGE clientB MESSAGE THIS a message from pid=%d ", getpid());
		error = send_message(temp);
	}
	if(count == 20){
